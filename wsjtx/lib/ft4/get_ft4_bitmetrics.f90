subroutine get_ft4_bitmetrics(cd,imetric,bitmetrics,badsync)
         ...
         do ib=0,ibmax
            ...
            if(nseq.eq.1) then
               den=max(maxval(s2(0:nt-1),one(0:nt-1,ibmax-ib)), &
                       maxval(s2(0:nt-1),.not.one(0:nt-1,ibmax-ib)))
               if(den.gt.0.0) then
                  bitmetrics(ipt+ib,4)=bm/den
               else
                  bitmetrics(ipt+ib,4)=0.0
               endif
            endif
         enddo
      enddo
...
! has the largest absolute value
   do i=1,2*NN
      if(abs(bitmetrics(i,1)).ge.abs(bitmetrics(i,2)) .and. &
         abs(bitmetrics(i,1)).ge.abs(bitmetrics(i,3))) then
         bitmetrics(i,5)=bitmetrics(i,1)
      elseif(abs(bitmetrics(i,2)).ge.abs(bitmetrics(i,3))) then
         bitmetrics(i,5)=bitmetrics(i,2)
      else
         bitmetrics(i,5)=bitmetrics(i,3)
      endif
   enddo
   ...
   call normalizebmet(bitmetrics(:,5),2*NN)
   return
