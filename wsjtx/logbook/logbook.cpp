...
void LogBook::match (QString const& call, QString const& mode, QString const& grid,
                     ...
                     bool& CQZoneB4,
                     bool& ITUZoneB4,
                     QString const& currentBand,
                     bool* out_stateB4,
                     bool* out_stateB4onBand) const
{
  if (call.size() > 0)
    {
      auto const& mode_to_check = (config_ && !config_->highlight_by_mode ()) ? QString {} : mode;
      callB4 = worked_before_.call_worked (call, mode_to_check, currentBand);
      gridB4 = worked_before_.grid_worked(grid, mode_to_check, currentBand);
      auto const& countryName = looked_up.entity_name;
      if (countryName.size ())
        {
          countryB4 = worked_before_.country_worked (countryName, mode_to_check, currentBand);
          continentB4 = worked_before_.continent_worked (looked_up.continent, mode_to_check, currentBand);
          CQZoneB4 = worked_before_.CQ_zone_worked (looked_up.CQ_zone, mode_to_check, currentBand);
          ITUZoneB4 = worked_before_.ITU_zone_worked (looked_up.ITU_zone, mode_to_check, currentBand);
        }
      else
        {
  ...
  if (out_stateB4 || out_stateB4onBand)
    {
      auto const& mode_to_check = (config_ && !config_->highlight_by_mode ()) ? QString {} : mode;
      // Try AE5TC method (callsign database) first
      auto st = USStates::callsignToState (call);
      if (st.isEmpty () && looked_up.primary_prefix == "K" && grid.size () >= 4)
        {
          // Fall back to NJ0A method (grid-to-state mapping),
          // but only use it if it yields a single state
          auto grid_st = worked_before_.countries ()->findState (grid.left (4));
      ...
      auto const is_tracked_state = st.size () == 2 && USStates::allStates ().contains (st);
      if (out_stateB4)
        {
          *out_stateB4 = true;
          if (is_tracked_state)
            {
              *out_stateB4 = worked_before_.state_worked (st, mode_to_check, {});
            }
        }
      if (out_stateB4onBand)
        {
          *out_stateB4onBand = true;
          if (is_tracked_state && currentBand.size ())
            {
              *out_stateB4onBand = worked_before_.state_worked (st, mode_to_check, currentBand);
            }
        }
    }
}
                               ...
                               QString const& satellite, QString const& satmode, QString const& freqRx)
{
  ...
  bool state_from_exchange = false;
  if (xRcvd.size ()) {
  ...
  if ((hisCall.startsWith("A") or hisCall.startsWith("K") or hisCall.startsWith("N") or hisCall.startsWith("W"))
      && !state_from_exchange && config_->GridMap ())
    {
      QString st = "";
      if (hisGrid.length() >= 4) {
        st = countries ()->findState (hisGrid);                                       // NJ0A method (grid-to-state mapping)
        if (st.size() != 2 || st == "**") st = USStates::callsignToState(hisCall);   // AE5TC method as fall back (callsign database)
      } else {
        st = USStates::callsignToState(hisCall);           // use AE5TC method when logging without grid
      }
        if (st.size () == 2 && st != "**")  t += " <state:" + QString::number (st.size ()) + ">" + st;
    }
  return t.toLatin1();
}
