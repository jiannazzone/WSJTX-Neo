...
// Parse ADIF STATE (used when appending log records; loader uses extractField in anonymous namespace).
static QString extract_adif_state (QString const& record)
{
  int fieldNameIndex = record.indexOf ("<STATE:", 0, Qt::CaseInsensitive);
  if (fieldNameIndex < 0) return {};
  int closingBracketIndex = record.indexOf ('>', fieldNameIndex);
  int fieldLengthIndex = record.indexOf (':', fieldNameIndex);
  if (fieldLengthIndex < 0 || closingBracketIndex < 0 || closingBracketIndex < fieldLengthIndex)
    return {};
  int dataTypeIndex = record.indexOf (':', fieldLengthIndex + 1);
  ...
  bool ok = false;
  int fieldLength = record.mid (fieldLengthIndex + 1, fieldLengthCharCount).toInt (&ok);
  if (!ok || fieldLength <= 0) return {};
  return record.mid (closingBracketIndex + 1, fieldLength).left (2).toUpper ();
}
...
struct worked_entry
{
  explicit worked_entry (QString const& call, QString const& grid, QString const& band
                         ...
                         , int CQ_zone, int ITU_zone, QString const& state = {})
    : call_ {call}
    , grid_ {grid}
    , band_ {band}
    , mode_ {mode}
...
std::size_t hash_value (worked_entry const& we)
{
  ...
  boost::hash_combine (seed, we.state_);
  return seed;
}
  ...
  indexed_by<
    ...
    ordered_non_unique<tag<ITU_zone_band>,
                       composite_key<worked_entry,
                                     member<worked_entry, int, &worked_entry::ITU_zone_>,
                                     member<worked_entry, QString, &worked_entry::band_> > >,
    // US state (ADIF STATE) + mode + band
    ordered_non_unique<tag<state_mode_band>,
                       composite_key<worked_entry,
                                     member<worked_entry, QString, &worked_entry::state_>,
                                     member<worked_entry, QString, &worked_entry::mode_>,
                                     member<worked_entry, QString, &worked_entry::band_> > >,
    // US state + band
    ordered_non_unique<tag<state_band>,
                       composite_key<worked_entry,
                                     member<worked_entry, QString, &worked_entry::state_>,
                                     member<worked_entry, QString, &worked_entry::band_> > > >
                                       ...
                                       // with terminator
                  {
                    ...
                    if (call.size ()) // require CALL field before we
                                      // will parse a record
                      {
                        ...
                        auto grid = extractField (record, "GRIDSQUARE").left (4).toUpper ();
                        auto state = extractField (record, "STATE").left (2).toUpper ();
                        // Derive state from callsign database or grid when STATE field is missing
                        if (state.isEmpty () && entity.primary_prefix == "K")
                          {
                            state = USStates::callsignToState (call);
                            if (state.isEmpty () && grid.size () >= 4)
                              {
                                auto grid_st = prefixes->findState (grid);
                                if (grid_st.size () == 2 && grid_st != "**")
                                  {
                                    state = grid_st;
                                  }
                              }
                          }
                        // Only track the 50 US states (ignore territories like PR, VI, GU)
                        if (!USStates::allStates ().contains (state))
                          {
                            state.clear ();
                          }
                        worked.emplace (call.toUpper ()
                                        , grid // not interested in 6-digit grids
  ...
  if (call.size ())
    {
      ...
      auto state = extract_adif_state (QString::fromUtf8 (ADIF_record));
      // Derive state from grid or callsign database when STATE field is missing
      if (state.isEmpty () && entity.primary_prefix == "K")
        {
          if (state.isEmpty () && grid.size () >= 4)
            {
              auto grid_st = m_->prefixes_.findState (grid.left (4));
              if (grid_st.size () == 2 && grid_st != "**")
                {
                  state = grid_st;
                }
      ...
      m_->worked_.emplace (call.toUpper (), grid.left (4).toUpper (), band.toUpper (), mode.toUpper ()
                           , entity.entity_name, entity.continent, entity.CQ_zone, entity.ITU_zone, state);
    }
  return true;
}
...
bool WorkedBefore::state_worked (QString const& state, QString const& mode, QString const& band) const
{
  auto const st = state.left (2).toUpper ();
  if (st.size () < 2) return true; // no state → do not treat as "new state"
  if (mode.size ())
    {
      if (band.size ())
        {
          return m_->worked_.get<state_mode_band> ().end ()
            != m_->worked_.get<state_mode_band> ().find (std::make_tuple (st, mode.toUpper (), band.toUpper ()));
        }
      return m_->worked_.get<state_mode_band> ().end ()
        != m_->worked_.get<state_mode_band> ().find (std::make_tuple (st, mode.toUpper ()));
    }
  if (band.size ())
    {
      return m_->worked_.get<state_band> ().end ()
        != m_->worked_.get<state_band> ().find (std::make_tuple (st, band.toUpper ()));
    }
  return m_->worked_.get<state_band> ().end ()
    != m_->worked_.get<state_band> ().find (st);
}
