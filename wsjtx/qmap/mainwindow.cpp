...
// Sized at the wide-mode upper bound so disk-load (getfile.cpp) can
// fread the full 60 s file at any supported rate. Footprint: ~61 MB
// at 256 kHz vs 23 MB at 96 kHz; fine for the host machine class
// QMAP runs on. Active loops touch only the first 2*60*activeRateHz()
// entries.
...
//  ui->decodedTextBrowser->clear();
  ...
  // Wide-waterfall tick spacing — exclusive action group (View menu).
  // KB2SA suggestion 2026-05-10: hardcoded 5 kHz cramped on smaller
  // screens. Initial check state synced to WideGraph's persisted value
  // below, after m_wide_graph_window is constructed.
  ...
  // When running with --config <path>, append the INI's basename to the
  // window title so two QMAPs side-by-side are visually distinguishable
  // (e.g. "QMAP — qmap-A" vs "QMAP — qmap-B"). Default path = no suffix.
  {
    QString title = program_title ();
    const QString default_path = m_appDir + "/qmap.ini";
...
// Read items for fAddComboBox
  ...
  // Mirror "Show callsigns on Waterfall": menu action ↔ WideGraph row
  // checkbox stay in sync from either direction. Initialise the menu
  // action from WideGraph's persisted state at startup.
  ...
  // Tick spacing: sync menu check state to WideGraph's persisted value,
  // then route each action's trigger to setTickSpacingKhz. One-way
  // (menu → WideGraph) since there's no other UI surface for this knob.
  {
  ...
  // Regression-harness hook: when --open <path> was given on the
  // CLI, run the same load path that File→Open uses, but without
  // the QFileDialog. Schedule it 1s after the GUI is up so the
  // Fortran init has finished. The disk-load loop in diskDat()
  // runs to completion, decodes flow to the GUI (and to
  // --decode-log if that's also set), then we return — the harness
  // can simply wait a fixed time and kill the process.
...
void MainWindow::writeSettings()
{
  ...
  // Linrad sample-rate mode + endpoint live in their own top-level
  // [Linrad] section so main.cpp can read them BEFORE MainWindow
  // construction (it knows nothing about the Common group).
  {
...
void MainWindow::readSettings()
{
  ...
  // Read [Linrad]/sample_rate_mode for the DevSetup combo state. The
  // actual rate-selection logic happens in main.cpp before this code
  // runs (qmap_runtime::activeRateHz() already reflects the choice);
  // we just need to mirror the combo state for the UI. Same applies
  // to host/tcp_port — main.cpp and WideGraph already read them
  // directly from the file at construction; here we just cache for
  // round-tripping through the DevSetup dialog.
  {
    SettingsGroup linrad_g {&settings, "Linrad"};
...
void MainWindow::guiUpdate()
{
  ...
  if(decodes_.ndecodes > m_fetched) {
    ...
    while(m_fetched<decodes_.ndecodes) {
      ...
      // WideGraph waterfall overlay: parse the freq + sender callsign
      // out of the decode line and push to the WideGraph's label list.
      // Decode line format (whitespace-separated, after trim()):
      //   "<HHMM> <freq_kHz> <fQSO_kHz> <DT> <SNR> <mode> <message>"
      //   e.g. "000100  177.771  176.3   2.48  -15  60A  CQ K1JT FN20"
      // We want column 2 (freq, kHz audio offset) for x-positioning,
      // and the SENDER's callsign from the message field. Sender is
      // either token[1] of the message ("CQ K1JT FN20" -> K1JT) or
      // token[1] of a directed call ("K1JT N6NU CN87" -> N6NU, since
      // token[0] is the recipient).
      {
        const QStringList cols = QString(t).split(QRegularExpression("\\s+"),SkipEmptyParts);
        if (cols.size() >= 7) {
          bool ok = false;
          const double freq_khz = cols[1].toDouble(&ok);
          if (ok) {
            // Message starts at column 6 (0-indexed). For "CQ X Y..."
            // sender is X (one past CQ); else sender is the second
            // token of the directed message (one past recipient).
            QString sender;
            if (cols[6] == "CQ" && cols.size() >= 8) {
              if (cols[7] == "DX" && cols.size() >= 9) sender = cols[8];
              else                                     sender = cols[7];
            } else if (cols.size() >= 8) {
              sender = cols[7];
            }
            if (!sender.isEmpty()) {
              m_wide_graph_window->addDecodeLabel(freq_khz, sender);
            }
          }
        }
      }
      // Regression-harness tap: when --decode-log <path> was passed
      // on the CLI, mirror each decode line out to that file. Format
      // matches the GUI's decode line (UTC + dT + freq + SNR + mode +
      // message), unmodified, one decode per line. The Python harness
      // in /dev/Q65/testfiles/ parses these and cross-checks against
      // the mapsim manifest's known-truth callsigns.
      if (!qmap_runtime::g_decode_log_path.isEmpty()) {
        QFile log(qmap_runtime::g_decode_log_path);
        if (log.open(QIODevice::Append | QIODevice::Text)) {
          QTextStream s(&log);
      ...
      // --exit-after-decode quit is now consolidated into
      // decoderFinished() (fires after every full decoder cycle,
      // success or no-decode). Per-line tap not needed.
      m_fetched++;
      m_nline++;
      QTextCursor cursor(ui->decodedTextBrowser->document()->findBlockByLineNumber(m_nline-1));
      QTextBlockFormat f = cursor.blockFormat();
