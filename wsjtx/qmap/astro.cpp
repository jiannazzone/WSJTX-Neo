  ...
  m_settings_filename {settings_filename}
{
  ...
  m_AzElDir0="";
  // Restore CFOM toggle state (off by default — most operators don't
  // run self-Doppler IQ shift unless deliberately listening for echoes).
  ui->cbCFOM->setChecked(settings.value("CFOMEnabled", false).toBool());
  // Hidden 2026-05-09 — IQ pre-shift causes Q65 decode to silently
  // fail off-air even with k0 candidate-shift tracking the cfom shift.
  // Plumbing is intact (commons.h::nCFOM, datcom_ → /npar/, qmapa.f90
  // call site commented). To re-enable for further debugging, remove
  // this line AND uncomment the two `if (nCFOM .ne. 0...)` blocks in
  // qmapa.f90 (cfom call + candidate f0 shift).
...
Astro::~Astro()
{
  ...
  settings.setValue ("CFOMEnabled", ui->cbCFOM->isChecked());
  delete ui;
}

bool Astro::isCFOMEnabled() const
{
  return ui && ui->cbCFOM && ui->cbCFOM->isChecked();
}
...
void Astro::astroUpdate(QDateTime t, QString mygrid, QString azelDir, double xavg)
{
  ...
  // Ensure a 6‑character buffer for Fortran, even if mygrid is empty
  QString mygrid6 = (mygrid + "      ").mid(0, 6);

  // Keep the QByteArray alive while calling Fortran
  QByteArray mygridBytes = mygrid6.toLatin1();
  const char *grid = mygridBytes.constData();
