subroutine read_qm(fname,iret)
  ...
  use qmap_params, only: MAX_NSMAX, MAX_NFFT
  include 'njunk.f90'
  parameter(NMAX=60*96000,NFFT=32768)      !active 96 kHz baseline
  character*(*) fname
...
! COMMON sized at MAX (256 kHz wide-mode upper bound); see recvpkt.f90.
  common/datcom/dd(2,MAX_NSMAX),ss(400,MAX_NFFT),savg(MAX_NFFT),        &
