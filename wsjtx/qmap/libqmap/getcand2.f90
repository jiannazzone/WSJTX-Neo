...
! Get candidates for Q65 decodes, based on presence of sync tone.
  ...
  use qmap_params, only: nrate_active, nfft_active, MAX_NFFT
...
! ss / savg0 are dummies; assumed-size matches the runtime active spectrum.
  real ss(400,*)                        !Symbol spectra
  real savg0(*)                         !Average spectrum (dummy)
! Local savg sized at MAX_NFFT so it covers any active size.
  real savg(MAX_NFFT)                   !Average spectrum (local working copy)
  ...
  if(savg0(nfft_active).eq.0) return    !Avoid a program crash when spectrum is empty (was savg0(NFFT))

  savg(1:nfft_active)=savg0(1:nfft_active)  !Save the original spectrum
  nlen=nfft_active/nseg                     !was NFFT/nseg
  ...
  df=real(nrate_active)/nfft_active      !was: 96000.0/NFFT then nrate_active/NFFT
  ...
  i1=1
  i2=nfft_active-nbw-nguard             !was: NFFT-nbw-nguard
  if(nagain.ge.1) then
...
! Sync tone is present, we have a candidate for decoding
           ...
           ib=min(i0+nbw+nguard,nfft_active)
           savg(ia:ib)=0.
