...
!  use iso_fortran_env, only: int16
  use qmap_params, only: nrate_active, nfft_active, MAX_NFFT_BIG, nfft_big_active

  parameter (MAXFFT1=5376000)              !56*96000 — historical baseline (active size = nfft_big_active)
  ...
  integer offset
! ca sized at MAX_NFFT_BIG (56*256000=14,336,000) to match fftbig.f90's
! common/cacb/ declaration; the FFT plan in fftbig is built at the
! active size nfft_big_active so reads/writes here only touch that
! prefix.
  complex ca(MAX_NFFT_BIG)                 !FFT of raw I/Q data from Linrad
  complex cx(0:MAXFFT2-1),cz(0:MAXFFT2)
  real*4 half_band_khz                     != 0.5 * nrate_active / 1000 (was hardcoded 48.0)
...
! Find best frequency from sync_dat, the "orange sync curve".
  df3=real(nrate_active)/nfft_active       !symspec NFFT bin width (was 96000.0/32768.0)
  ipk=(1000.0*f0-1.0)/df3
  ! Wide-mode: 48.0 was the 96 kHz half-band; now scale with the active rate.
  if(nagain.ge.2) ipk = nint(1000.0*(fqso-nkhz_center+0.5*real(nrate_active)/1000.0)/df3)
  nfft1=nfft_big_active                    !was: nfft1=MAXFFT1=5376000
  nfft2=MAXFFT2
  df=real(nrate_active)/NFFT1              !was: 96000.0/NFFT1 — long-FFT bin width
  nh=nfft2/2
  k0=nint((ipk*df3-1000.0)/df)
  if(k0.lt.nh .or. k0.gt.nfft_big_active-nfft2+1) go to 900     !was: MAXFFT1-nfft2+1
  fac=1.0/nfft2
...
!          If this is a dupe, don't save it again:
     ...
     ! Half-band-width in kHz: was hardcoded -48.0 (= 0.5 * 96 kHz baseline);
     ! at wide-mode we need 0.5 * nrate_active / 1000 so the displayed freq
     ! aligns with the cfreq-anchored waterfall labels.
     half_band_khz = 0.5 * real(nrate_active) / 1000.0
     nq65df=nint(1000*(0.001*k0*df+nkhz_center-half_band_khz+1.000-1.27046-ikhz))-nfcal
     ...
     frx=0.001*k0*df+nkhz_center-half_band_khz+1.0 - 0.001*nfcal
     ! When CFOM is active, qmapa already shifted the IQ by -ndop00/2,
     ! so frx is already the dial-equivalent freq. Subtracting ndop00/2
     ! again would double-correct, so skip that term.
     if (nCFOM .ne. 0) then
        fsked=frx - 0.001*offset
     else
