...
! Test for presence of Q65 sync tone
  ...
  use qmap_params, only: nrate_active, nfft_active
  parameter (LAGMAX=33)
! ss is a dummy arg; assumed-size matches caller's active spectrum
...
!  if(ntrperiod.ne.60) return
  ...
  i1=max(1,i0-m)
  i2=min(nfft_active,i0+m)         !was: min(NFFT,i0+m)
  ccf=0.
...
! Q: Should we use weighted sums, perhaps a Lorentzian peak?
  ...
  sq=0.
  nsum=0
  fpk=0.001*i0*real(nrate_active)/nfft_active           !was: 0.001*i0*96000.0/32768.0 + 32.0
  ! +32.0 was a 96-kHz-baseline-era constant (likely a band-edge shift)
  ! that didn't carry meaning at wider rates. fpk is debug-only here
  ! (used in the write() at line ~60 below, which is commented out);
  ! dropped the offset to match getcand2.f90's convention so anyone
  ! uncommenting the write() sees a consistent kHz-from-DC value.
