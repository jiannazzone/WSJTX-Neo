...
void EQSL::networkReply(QNetworkReply *incomingReply)
{
    if (incomingReply->request().originatingObject() != this) return;

    if (QNetworkReply::NoError != incomingReply->error ()) {
      printf ("eqsl upload error:%d\n",incomingReply->error ());
    }
    else {
      QString serverResponse = incomingReply->readAll();
    }

    incomingReply->deleteLater ();
    m_in_progress = false;
}
