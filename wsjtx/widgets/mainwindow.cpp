  ...
  m_useDarkStyle {false}
{
  ...
  // Decoded-callsign overlay (N6NU port from QMAP 2026-05-11): each
  // decode line flowing through DisplayText emits decodedCallsign;
  // WideGraph receives, dedupes, ages, and pushes to the plotter.
  connect(ui->decodedTextBrowser, &DisplayText::decodedCallsign,
          m_wideGraph.data (), &WideGraph::addDecodeLabel);
  // RxFrequency window (decodedTextBrowser2) also flows decodes; the
  // same callsign appearing in both windows just refreshes the same
  // entry (dedupe by callsign in addDecodeLabel).
  connect(ui->decodedTextBrowser2, &DisplayText::decodedCallsign,
          m_wideGraph.data (), &WideGraph::addDecodeLabel);

  // View menu wiring for the overlay toggle + persistence selector.
  // Mirror WideGraph's persisted state on startup; route menu clicks
  // through WideGraph's setters so persistence stays consistent.
  ui->actionCallsign_overlay_enabled->setChecked(
      m_wideGraph->decodeLabelsEnabled());
  connect(ui->actionCallsign_overlay_enabled, &QAction::toggled,
          m_wideGraph.data (), &WideGraph::setDecodeLabelsEnabled);
  connect(m_wideGraph.data (), &WideGraph::decodeLabelsEnabledChanged,
          ui->actionCallsign_overlay_enabled, &QAction::setChecked);
  {
    QActionGroup* persistGroup = new QActionGroup(this);
    persistGroup->setExclusive(true);
    ui->actionCallsign_persist_1->setActionGroup(persistGroup);
    ui->actionCallsign_persist_2->setActionGroup(persistGroup);
    ui->actionCallsign_persist_3->setActionGroup(persistGroup);
    ui->actionCallsign_persist_4->setActionGroup(persistGroup);
    ui->actionCallsign_persist_5->setActionGroup(persistGroup);
    const int p = m_wideGraph->decodeLabelPeriods();
    if      (p == 1) ui->actionCallsign_persist_1->setChecked(true);
    else if (p == 3) ui->actionCallsign_persist_3->setChecked(true);
    else if (p == 4) ui->actionCallsign_persist_4->setChecked(true);
    else if (p == 5) ui->actionCallsign_persist_5->setChecked(true);
    else             ui->actionCallsign_persist_2->setChecked(true);  // 2 = default
    auto* wg = m_wideGraph.data();
    connect(ui->actionCallsign_persist_1, &QAction::triggered,
            wg, [wg]{ wg->setDecodeLabelPeriods(1); });
    connect(ui->actionCallsign_persist_2, &QAction::triggered,
            wg, [wg]{ wg->setDecodeLabelPeriods(2); });
    connect(ui->actionCallsign_persist_3, &QAction::triggered,
            wg, [wg]{ wg->setDecodeLabelPeriods(3); });
    connect(ui->actionCallsign_persist_4, &QAction::triggered,
            wg, [wg]{ wg->setDecodeLabelPeriods(4); });
    connect(ui->actionCallsign_persist_5, &QAction::triggered,
            wg, [wg]{ wg->setDecodeLabelPeriods(5); });
    // WideGraph spin box → menu radio: every supported value (1..5)
    // now matches exactly one radio, so the menu always stays in sync
    // with the spin box.
    connect(wg, &WideGraph::decodeLabelPeriodsChanged,
            this, [this](int n) {
                if      (n == 1) ui->actionCallsign_persist_1->setChecked(true);
                else if (n == 2) ui->actionCallsign_persist_2->setChecked(true);
                else if (n == 3) ui->actionCallsign_persist_3->setChecked(true);
                else if (n == 4) ui->actionCallsign_persist_4->setChecked(true);
                else if (n == 5) ui->actionCallsign_persist_5->setChecked(true);
            });

    // exclusive group, persisted via WideGraph QSettings.
    ...
    switch (m_wideGraph->decodeLabelFontSize()) {
    ...
    // Overlay transparency: exclusive group, four presets (None=255,
    // Low=245, Medium=215, High=175). Persisted via WideGraph.
    QActionGroup* transparencyGroup = new QActionGroup(this);
    transparencyGroup->setExclusive(true);
    ui->actionCallsign_transparency_None  ->setActionGroup(transparencyGroup);
    ui->actionCallsign_transparency_Low   ->setActionGroup(transparencyGroup);
    ui->actionCallsign_transparency_Medium->setActionGroup(transparencyGroup);
    ui->actionCallsign_transparency_High  ->setActionGroup(transparencyGroup);
    {
      const int a = m_wideGraph->decodeLabelAlpha();
      if      (a == 175) ui->actionCallsign_transparency_High  ->setChecked(true);
      else if (a == 215) ui->actionCallsign_transparency_Medium->setChecked(true);
      else if (a == 245) ui->actionCallsign_transparency_Low   ->setChecked(true);
      else               ui->actionCallsign_transparency_None  ->setChecked(true);
    }
    connect(ui->actionCallsign_transparency_None,   &QAction::triggered,
            wg, [wg]{ wg->setDecodeLabelAlpha(255); });
    connect(ui->actionCallsign_transparency_Low,    &QAction::triggered,
            wg, [wg]{ wg->setDecodeLabelAlpha(245); });
    connect(ui->actionCallsign_transparency_Medium, &QAction::triggered,
            wg, [wg]{ wg->setDecodeLabelAlpha(215); });
    connect(ui->actionCallsign_transparency_High,   &QAction::triggered,
            wg, [wg]{ wg->setDecodeLabelAlpha(175); });

    // Seed the active callsign in case there's already one in DX Call
    // when the menu wires up (e.g., restoring from a saved session).
    if (!m_hisCall.isEmpty()) wg->setActiveCallsign(m_hisCall);
  }
  ...
  connect (&m_logBook, &LogBook::finished_loading, [this] (int record_count, QString const& error) {
      if (error.size ())
        {
// temporarily disable error message because download doesn't work with Qt6
//          MessageBox::warning_message (this, tr ("Error Scanning ADIF Log"), error);
  ...
  set_dateTimeQSO(-1);
  connect(txMsgButtonGroup,SIGNAL(idClicked(int)),SLOT(set_ntx(int)));
  ...
  // 50 ms delay is required for Qt 6.5.0
...
//    QTimer::singleShot (0, this, SLOT (not_GA_warning_message ()));
  ...
  QtConcurrent::run([=](){
    USStates::ensureBinaryUpToDate();   // regenerate .bin if needed

    QMetaObject::invokeMethod(this, []() {
      USStates::reloadCallsignDatabase();
      USStates::preloadDatabaseAsync();
    });
  });
...
// m_settings->setValue("ShMsgs",m_bShMsgs);
  ...
  m_settings->setValue ("QRG1FT2", QRG1FT2);
  m_settings->setValue ("QRG2FT2", QRG2FT2);
  m_settings->setValue ("QRG3FT2", QRG3FT2);
  m_settings->setValue ("QRG4FT2", QRG4FT2);
  m_settings->setValue ("QRG5FT2", QRG5FT2);
  m_settings->setValue ("QRG6FT2", QRG6FT2);
  m_settings->setValue ("QRG7FT2", QRG7FT2);
  m_settings->setValue ("QRG8FT2", QRG8FT2);
...
void MainWindow::readSettings()
{
  ...
  QRG1FT2=m_settings->value("QRG1FT2", false).toBool();
  QRG2FT2=m_settings->value("QRG2FT2", false).toBool();
  QRG3FT2=m_settings->value("QRG3FT2", false).toBool();
  QRG4FT2=m_settings->value("QRG4FT2", false).toBool();
  QRG5FT2=m_settings->value("QRG5FT2", false).toBool();
  QRG6FT2=m_settings->value("QRG6FT2", false).toBool();
  QRG7FT2=m_settings->value("QRG7FT2", false).toBool();
  QRG8FT2=m_settings->value("QRG8FT2", false).toBool();
...
void MainWindow::set_application_font (QFont const& font)
{
  ...
  if (ui->actionUse_Dark_Style->isChecked()) {
      ...
      if (!f.exists())   {
          ...
          qApp->setFont (font);
          QString ss;
          qDebug() << "File open result:" <<  f.open(QFileDevice::ReadOnly | QFileDevice::Text);
          QTextStream ts(&f);
...
// Get power, spectrum, and ihsym
  ...
  if(m_mode=="FT2" && m_diskData) bCallDecoder=true;  // why do we need this?
...
//  ::memcpy(dec_data.params.hiscall,(Radio::base_callsign (hisCall) +  "            ").toLatin1 ().constData (), sizeof dec_data.params.hiscall);
  ...
  if(bmsk144 and (line[0]!=0)) {
    ...
    if (!filtered or m_config.filters_for_Wait_and_Pounce_only()) {
        ...
        // Keep his grid during a QSO for mapping US States
        if (m_auto && decodedtext.string().contains(m_config.my_callsign () + " " + m_hisCall)) {
          QSOgrid = m_hisGrid;
        } else {
          QSOgrid = "";
        }
        ...
        ui->decodedTextBrowser->displayDecodedText (decodedtext, m_config.my_callsign (), m_mode, m_config.DXCC (),
          ...
          haveFSpread, fSpread, bDisplayPoints, m_points, distance, m_muted, QSOgrid);
        if(m_position != 0) ui->decodedTextBrowser->horizontalScrollBar()->setValue(m_position);
        ...
        if (m_mode=="MSK144" && text.mid(22).contains(m_baseCall + " " + m_hisCall + " 73")) {
            ui->decodedTextBrowser2->displayDecodedText (decodedtext, m_config.my_callsign (), m_mode, m_config.DXCC (),
              m_logBook, m_currentBand, m_config.ppfx (), false, false, 0.0, false, -99, "", m_muted, QSOgrid);
        }
    }
...
void MainWindow::on_actionSettings_triggered()           // Setup Dialog (Settings dialog)
{
  ...
  m_config.read_callsign_states_info();
...
{
  monitor (false);
  if (m_wideGraph) m_wideGraph->clearDecodeLabels();
...
void MainWindow::read_wav_file (QString const& fname)
{
  ...
  m_wav_future_watcher.setFuture (QtConcurrent::run ([this, fname] {
    ...
    if(ok) {
      ...
      qDebug () << "mainwindow: bytes_per_frame " << bytes_per_frame;
...
//Left (Band activity) window
    ...
    if(!m_bFastDone) {
      ui->decodedTextBrowser->displayDecodedText (decodedtext, m_config.my_callsign (), m_mode, m_config.DXCC (),
         m_logBook, m_currentBandPeriod, m_config.ppfx (), false, false, 0.0, false, -99, "", m_muted, QSOgrid);
      if(m_position != 0) ui->decodedTextBrowser->horizontalScrollBar()->setValue(m_position);
    }
      ...
      if(!bAvgMsg) {
        if(m_mode=="FT8" and SpecOp::FOX == m_specOp) {
          ...
          if (SpecOp::HOUND == m_specOp) {
            ...
            if (!callsign.isEmpty()) {
              if (!m_config.ShowOTP() || otp == QLatin1String("000000")) filtered = true;
      ...
      if (bDisplayRight && !block_right_display) {
        ...
        if(!pounce && (!m_bBestSPArmed or (m_mode!="FT4" and m_mode!="FT2"))) {
          ...
          ui->decodedTextBrowser2->displayDecodedText (decodedtext0, m_config.my_callsign (), m_mode, m_config.DXCC (),
            m_logBook, m_currentBand, m_config.ppfx (), false, false, 0.0, bDisplayPoints, m_points, "", m_muted, QSOgrid);
        }
...
{
  // For WSPR and FST4W modes, also require "Upload spots" to be enabled
  if ((m_mode == "WSPR" /* || m_mode == "FST4W" */) && !m_uploadWSPRSpots) return;
...
//      w34=w.at(nw-2);
  ...
  if (s1!=s2 and !message.isTX()) {
    ...
    if (!s2.contains(m_baseCall) or m_mode=="MSK144") {  // Taken care of elsewhere if for_us and slow mode
      // Keep his grid during a QSO for mapping US States
      if (m_auto && message.clean_string ().contains(m_config.my_callsign () + " " + m_hisCall)) {
        QSOgrid = m_hisGrid;
      } else {
        QSOgrid = "";
      }
...
void MainWindow::genCQMsg ()
{
  ...
  if(my_callsign.size () && m_config.my_grid().size ()) {
    ...
    if((m_mode=="FT4" or m_mode=="FT8" or m_mode=="MSK144" or "Q65" == m_mode) and
...
void MainWindow::mousePressEvent(QMouseEvent *event)    // mouse press events
{
  ...
  // Toggle between FT8 and FT2 mode for the 8 selectable band hopping frequencies
  if(ui->cbQRG1->hasFocus() && (event->button() & Qt::RightButton)) {
    if (QRG1FT2) {
      QRG1FT2 = false;
    } else {
      QRG1FT2 = true;
    }
    ui->cbQRG1->clearFocus();
  }
  if(ui->cbQRG2->hasFocus() && (event->button() & Qt::RightButton)) {
    if (QRG2FT2) {
      QRG2FT2 = false;
    } else {
      QRG2FT2 = true;
    }
    ui->cbQRG2->clearFocus();
  }
  if(ui->cbQRG3->hasFocus() && (event->button() & Qt::RightButton)) {
    if (QRG3FT2) {
      QRG3FT2 = false;
    } else {
      QRG3FT2 = true;
    }
    ui->cbQRG3->clearFocus();
  }
  if(ui->cbQRG4->hasFocus() && (event->button() & Qt::RightButton)) {
    if (QRG4FT2) {
      QRG4FT2 = false;
    } else {
      QRG4FT2 = true;
    }
    ui->cbQRG4->clearFocus();
  }
  if(ui->cbQRG5->hasFocus() && (event->button() & Qt::RightButton)) {
    if (QRG5FT2) {
      QRG5FT2 = false;
    } else {
      QRG5FT2 = true;
    }
    ui->cbQRG5->clearFocus();
  }
  if(ui->cbQRG6->hasFocus() && (event->button() & Qt::RightButton)) {
    if (QRG6FT2) {
      QRG6FT2 = false;
    } else {
      QRG6FT2 = true;
    }
    ui->cbQRG6->clearFocus();
  }
  if(ui->cbQRG7->hasFocus() && (event->button() & Qt::RightButton)) {
    if (QRG7FT2) {
      QRG7FT2 = false;
    } else {
      QRG7FT2 = true;
    }
    ui->cbQRG7->clearFocus();
  }
  if(ui->cbQRG8->hasFocus() && (event->button() & Qt::RightButton)) {
    if (QRG8FT2) {
      QRG8FT2 = false;
    } else {
      QRG8FT2 = true;
    }
    ui->cbQRG8->clearFocus();
  }
  ...
  if (ui->pbBandHopping->hasFocus() && event->button() & Qt::RightButton) {  // Band Hopping button
    if(ui->pbBandHopping->isChecked()) {
      ...
      connect(audio, SIGNAL(stateChanged(QAudio::State)), this, SLOT(handleStateChanged(QAudio::State)));
      QFile *effect1 = new QFile(this);
      effect1->setFileName(QString("%1/%2").arg(audioPath, "Testing_long.wav"));
      effect1->open(QIODevice::ReadOnly);
      audio->start(effect1);
    }
...
void MainWindow::on_dxCallEntry_textChanged (QString const& call)
{
  ...
  m_hisCall = call;
  // DG2YCB 2026-05-12: mirror DX Call entry into WideGraph so the
  // matching waterfall label gets the bright-green active highlight.
  if (m_wideGraph) m_wideGraph->setActiveCallsign(call);
...
{
  if (m_config.EraseWhenModeChanged() && last_mode != m_mode) {
    ui->decodedTextBrowser->erase ();
    ui->decodedTextBrowser2->erase ();
    clearDX();                                  // clear dxCallEntry
    ui->dxGridEntry->clear ();                  // clear dxGridEntry
...
void MainWindow::p1ReadFromStdout()                        //p1readFromStdout
{
  ...
  while(p1.canReadLine()) {
    ...
    if(t.indexOf("<DecodeFinished>") >= 0) {
      ...
      // Upload to PSK Reporter if "Upload spots" is enabled and PSK Reporter is configured
      if (m_uploadWSPRSpots && m_config.spot_to_psk_reporter() && rxFields.count() >= 7) {
          // For WSPR, extract data directly from rxFields instead of using DecodedText parsing
          QString deCall = rxFields.at(5);
          QString deGrid = (rxFields.count() >= 8) ? rxFields.at(6) : QString {};
          int snr = rxFields.at(1).toInt();

          // RF frequency from decoder (field 3 is in MHz)
          Frequency rfFrequency = static_cast<Frequency>(rxFields.at(3).toDouble() * 1000000.0 + 0.5);

          // Create spot time from WSPR timestamp
          int h = rxFields.at(0).left(2).toInt();
          int m = rxFields.at(0).mid(2,2).toInt();
          QTime wsprTime(h, m, 0);
          QDateTime qSpotTime;
          int timeInSeconds = rxFields.at(0).toInt();
          if (timeInSeconds + int(m_TRperiod) < 236000) {
            qSpotTime = QDateTime(QDateTime::currentDateTimeUtc().date(), wsprTime, Qt::UTC);
          } else {
            qSpotTime = QDateTime((QDateTime::currentDateTimeUtc().addDays(-1)).date(), wsprTime, Qt::UTC);
          }

          // Send to PSK Reporter (only if grid is valid)
          if (deGrid.contains (grid_regexp)) {
            if (!m_psk_Reporter.addRemoteStation (deCall, deGrid, rfFrequency, QString {"WSPR"}, snr, qSpotTime)) {
              showStatusMessage (tr ("Spotting to PSK Reporter unavailable"));
            }
          }
      }
...
//    m_foxQSO[hc].rcvd = -99;              //Have not received R+rpt
    m_foxQSO[hc].rcvd = "";               //Have not received R+rpt
...
{
  QString t;
  t = t.asprintf("%3d%3d%3d",static_cast<int>(m_houndQueue.count()),
  ...
  while(!s.atEnd()) {
    ...
    if(line.contains("Tx1:")) {
      ...
      t = t.asprintf("%3d %3d %3d %3d %5d   ",
                ...
                static_cast<int>(m_foxQSOinProgress.count()),
                static_cast<int>(m_foxQSO.count()),
                static_cast<int>(m_loggedByFox.count()),m_tFoxTx);
...
{
#if defined(Q_OS_WIN)
    QFile::remove (QDir {QStandardPaths::writableLocation (QStandardPaths::AppLocalDataLocation)}.absoluteFilePath ("wsjtx_log_config.ini"));
#else
...
{
#if defined(Q_OS_WIN)
    static QFile f {QDir {QStandardPaths::writableLocation (QStandardPaths::AppLocalDataLocation)}.absoluteFilePath ("wsjtx_log_config.ini")};
#else
...
void MainWindow::on_actionDisable_event_logging_triggered()
{
    ...
    out << EventConfig;
    f.close();
    QFile::remove (QDir {QStandardPaths::writableLocation (QStandardPaths::AppLocalDataLocation)}.absoluteFilePath ("wsjtx_syslog.log"));
}
...
void MainWindow::check_band_limits()
{
  // UK stations must transmit below 5358 kHz or above 5362 kHz on 60m
  Frequency bw=50;  // FT8
  if(m_mode=="FT4") bw=70;
  if(m_mode=="FT2") bw=135;
  if(m_mode=="JT65") bw=180;
  if((m_config.my_callsign().startsWith("G") or m_config.my_callsign().startsWith("M")
      or m_config.my_callsign().startsWith("2")) && ((m_freqNominal + ui->TxFreqSpinBox->value()) > (5358000 - bw))
      && (m_freqNominal + ui->TxFreqSpinBox->value()) < 5362000) {
    QString band=m_config.bands()->find(m_freqNominal);
    if(band=="60m") {
      if (m_freqNominal + ui->TxFreqSpinBox->value() < 5361000) {
        ui->TxFreqSpinBox->setValue(5358000 - m_freqNominal - bw);
        m_wideGraph->setTxFreq(5358000 - m_freqNominal - bw);
      }
      if (m_freqNominal + ui->TxFreqSpinBox->value() > 5361000) {
...
void MainWindow::check_button_color()
{
  ...
  if (QRG1FT2) {
    ui->sbQRG1->setStyleSheet("QSpinBox{color: #000000; background-color: #ffacda}");
  } else {
    ui->sbQRG1->setStyleSheet("");
  }
  if (QRG2FT2) {
    ui->sbQRG2->setStyleSheet("QSpinBox{color: #000000; background-color: #ffacda}");
  } else {
    ui->sbQRG2->setStyleSheet("");
  }
  if (QRG3FT2) {
    ui->sbQRG3->setStyleSheet("QSpinBox{color: #000000; background-color: #ffacda}");
  } else {
    ui->sbQRG3->setStyleSheet("");
  }
  if (QRG4FT2) {
    ui->sbQRG4->setStyleSheet("QSpinBox{color: #000000; background-color: #ffacda}");
  } else {
    ui->sbQRG4->setStyleSheet("");
  }
  if (QRG5FT2) {
    ui->sbQRG5->setStyleSheet("QSpinBox{color: #000000; background-color: #ffacda}");
  } else {
    ui->sbQRG5->setStyleSheet("");
  }
  if (QRG6FT2) {
    ui->sbQRG6->setStyleSheet("QSpinBox{color: #000000; background-color: #ffacda}");
  } else {
    ui->sbQRG6->setStyleSheet("");
  }
  if (QRG7FT2) {
    ui->sbQRG7->setStyleSheet("QSpinBox{color: #000000; background-color: #ffacda}");
  } else {
    ui->sbQRG7->setStyleSheet("");
  }
  if (QRG8FT2) {
    ui->sbQRG8->setStyleSheet("QSpinBox{color: #000000; background-color: #ffacda}");
  } else {
    ui->sbQRG8->setStyleSheet("");
  }
}
...
{
    static QFile logfile {QDir {QStandardPaths::writableLocation (QStandardPaths::AppLocalDataLocation)}.absoluteFilePath ("wsjtx.log")};
...
void MainWindow::alertQSYmessage ()
{
  ...
  connect(audio, SIGNAL(stateChanged(QAudio::State)), this, SLOT(handleStateChanged(QAudio::State)));
  QFile *effect1 = new QFile(this);
  effect1->setFileName(QString("%1/%2").arg(audioPath, "/sounds/Message.wav"));
  effect1->open(QIODevice::ReadOnly);
  audio->start(effect1);
  play_DXcall = false;
}
