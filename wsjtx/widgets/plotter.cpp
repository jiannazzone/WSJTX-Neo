...
void CPlotter::paintEvent(QPaintEvent *)                                // paintEvent()
{
  ...
  // Decoded-callsign overlay (N6NU 2026-05-11): rendered last so it
  // sits on top of waterfall + bar markers. WideGraph manages the
  // list lifecycle (ageing per TR period); we just paint what's
  // currently set via setDecodeLabels().
...
void CPlotter::paintDecodeLabels(QPainter& painter)
{
  ...
  // order of x-position. List is bounded by WideGraph at kDecodeLabelMax.
  QList<DecodeLabel> sorted = m_decodeLabels;
  std::sort(sorted.begin(), sorted.end(),
            [this](const DecodeLabel& a, const DecodeLabel& b) {
                return XfromFreq(static_cast<float>(a.freq_hz))
                     < XfromFreq(static_cast<float>(b.freq_hz));
            });
  ...
  // FT8 signals are 50 Hz apart so callsign labels almost always
  // overlap. 6 stack rows handles a busy band. Each row is row_height
  // pixels tall — at 9 px font + 1 margin = 10 px, 6 rows = 60 px
  // off the top of the waterfall. Within the 30-px scale + 30 px of
  // typical waterfall height this is fine; if the waterfall is
  // shorter, top rows clip but the overlay is still readable.
  ...
  //   master == 255 ("None"): force every element to alpha 255, overriding
  //                           any base alpha the colour already carries
  //                           (BG 200/230, tick 200/220, etc.). This makes
  //                           "None" actually solid as the label implies.
  //   master  < 255 (Medium/High): scale each element's base alpha
  //                                proportionally so the relative layering
  //                                between BG / tick / text is preserved as
  //                                the overlay fades.
  ...
  for (const auto& l : sorted) {
    const int x = XfromFreq(static_cast<float>(l.freq_hz));
    ...
    for (; row < max_rows; ++row) {
      if (rect_x > row_right_edge[row] + 2) break;
    }
    if (row >= max_rows) continue;      // out of rows — drop
    ...
    //   1. Active callsign (= what's in WSJT-X's DX Call entry):
    //      light green, same green family as the "Receiving" indicator
    //   2. Even period: black with alpha
    //   3. Odd period:  white with alpha (lets dark text be readable)
    QColor bg;
    if (l.is_active) {
      bg = QColor(120, 220, 120, 230);   // bright "Receiving" green
    } else if (l.is_even_period) {
      bg = QColor(0, 0, 0, 200);
    } else {
      bg = QColor(255, 255, 255, 200);
    }
    painter.fillRect(rect, applyAlpha(bg));
    // Tick from label down to the actual signal x — anchors the eye
    // when stacking is deep. Match the callsign colour for cohesion.
    QColor tick_col;
    if (l.is_active)         tick_col = QColor(0, 130, 0, 230);
    else if (l.is_cq)        tick_col = QColor(255, 60, 60, 220);
    else if (l.is_even_period) tick_col = QColor(255, 230, 0, 200);
    else                     tick_col = QColor(20, 20, 20, 200);
    painter.setPen(applyAlpha(tick_col));
    painter.drawLine(x, y_top + row_height, x, y_top + row_height + 3);
    // Callsign text colour: pick for legibility against the bg above.
    //   - Active: black on green (high contrast)
    //   - CQ on dark bg (even period): red, eye-catching
    //   - CQ on light bg (odd period): dark red
    //   - Non-CQ on dark bg: yellow
    //   - Non-CQ on light bg: black
    QColor text_col;
    if (l.is_active) {
      text_col = QColor(0, 0, 0);
    } else if (l.is_cq) {
      text_col = l.is_even_period ? QColor(255,  115,   0)  // #ff4f00 — orange
                                  : QColor(180,   0,   0);
    } else {
      text_col = l.is_even_period ? Qt::yellow : QColor(20, 20, 20);
    }
    painter.setPen(applyAlpha(text_col));
