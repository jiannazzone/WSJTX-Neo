                                     ...
                                     int points, QString distance, bool alertsMuted, QString const& QSOgrid)
{
  ...
  // (N6NU 2026-05-11). MainWindow connects this to WideGraph. Skip
  // anonymous / "<...>" hash placeholders since they aren't useful
  // labels.
  if (!dxCall.isEmpty() && !dxCall.startsWith('<')) {
    emit decodedCallsign(static_cast<double>(decodedText.frequencyOffset()),
                         dxCall,
                         CQcall,
                         static_cast<int>(decodedText.timeInSeconds()));
  }
