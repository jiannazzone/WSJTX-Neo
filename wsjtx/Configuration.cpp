...
namespace
{
  QDataStream::Version constexpr decode_highlight_stream_version {QDataStream::Qt_5_12};

  bool load_decode_highlighting_v2 (QSettings * settings, DecodeHighlightingModel::HighlightItems & items)
  {
    auto encoded = settings->value ("DecodeHighlighting_v2").toString ();
    if (encoded.isEmpty ()) return false;
    QByteArray data = QByteArray::fromBase64 (encoded.toLatin1 ());
    QDataStream stream {&data, QIODevice::ReadOnly};
    stream.setVersion (decode_highlight_stream_version);
    stream >> items;
    return stream.status () == QDataStream::Ok;
  }

  void migrate_insert_us_state_highlights (DecodeHighlightingModel::HighlightItems & items)
  {
    using Highlight = DecodeHighlightingModel::Highlight;
    using HighlightInfo = DecodeHighlightingModel::HighlightInfo;
    for (auto const& hi : items)
      {
        if (hi.type_ == Highlight::USState || hi.type_ == Highlight::USStateBand)
          return;
      }
    HighlightInfo const us {Highlight::USState, true, {{0xaa, 0x00, 0x00}}, {{0xb4, 0x69, 0xff}}};
    HighlightInfo const usb {Highlight::USStateBand, true, {{0xaa, 0x00, 0x00}}, {{0xd2, 0xa5, 0xff}}};
    int idx = -1;
    for (int i = 0; i < items.size (); ++i)
      {
        if (items[i].type_ == Highlight::Grid)
          {
            idx = i;
            break;
          }
...
private:
  ...
  void after_callsign_states_downloaded();
  void error_during_callsign_states_download (QString const& reason);
  void read_callsign_states_info();
  ...
  Q_SLOT void on_adif_log_loaded (int record_count, QString cty_version, QString const& error);  //AE5TC
  Q_SLOT void on_CTY_download_button_clicked (bool);
  Q_SLOT void on_callsign_states_download_button_clicked (bool);
  ...
  QSettings * settings_;
  LogBook * logbook_;
  int adif_record_count_;  //AE5TC - track ADIF log entry count
...
bool Configuration::EraseWhenModeChanged() const { return m_->erase_when_mode_changed_;}
...
bool Configuration::alert_USState () const {return m_->alert_USState_;}
bool Configuration::alert_USStateOB () const {return m_->alert_USStateOB_;}
...
void Configuration::read_callsign_states_info ()
{
  m_->read_callsign_states_info ();
}

//AE5TC - Update ADIF record count display (called by MainWindow after LogBook loads)
void Configuration::set_adif_record_count (int count)
{
  m_->on_adif_log_loaded (count, QString{}, QString{});
}
  ...
  , default_audio_output_device_selected_ {false}
{
  ...
  // Preload the callsign states database asynchronously to avoid GUI blocking
  // on first decode that looks up a state
  if (USStates::binaryFileExists()) {
...
void Configuration::impl::initialize_models ()
{
  ...
  ui_->cbErase_when_mode_changed->setChecked(erase_when_mode_changed_);
  ...
  ui_->cbUSState->setChecked(alert_USState_);
  ui_->cbUSStateOB->setChecked(alert_USStateOB_);
...
void Configuration::impl::read_settings ()
{
  ...
  {
    auto encoded = settings_->value ("DecodeHighlighting_v2").toString ();
    if (!encoded.isEmpty ())
      {
        QByteArray data = QByteArray::fromBase64 (encoded.toLatin1 ());
  ...
  if (!load_decode_highlighting_v2 (settings_, highlight_items)) {
    highlight_items = settings_->value ("DecodeHighlighting", QVariant::fromValue (DecodeHighlightingModel::default_items ())).value<DecodeHighlightingModel::HighlightItems> ();
  }
  migrate_insert_us_state_highlights (highlight_items);
  ...
  erase_when_mode_changed_ = settings_->value("EraseWhenModeChanged", false).toBool();
  ...
  alert_USState_ = settings_->value("alert_USState",false).toBool ();
  alert_USStateOB_ = settings_->value("alert_USStateOB",false).toBool ();
...
void Configuration::impl::write_settings ()
{
  ...
  settings_->setValue ("FrequenciesForRegionModes_v2", QVariant::fromValue (frequencies_.frequency_list ()));
  {
    ...
    s << decode_highlighing_model_.items ();
    settings_->setValue ("DecodeHighlighting_v2", QString::fromLatin1 (data.toBase64 ()));
  }
  ...
  settings_->setValue ("EraseWhenModeChanged", erase_when_mode_changed_);
  ...
  settings_->setValue ("alert_USState", alert_USState_);
  settings_->setValue ("alert_USStateOB", alert_USStateOB_);
...
int Configuration::impl::exec ()
{
  ...
  //AE5TC - Update ADIF rescan button with current record count
  if (adif_record_count_ > 0) {
    ui_->rescan_log_push_button->setText(QString{"Rescan ADIF Log (%1)"}.arg(adif_record_count_));
  }
...
void Configuration::impl::accept ()
{
  ...
  erase_when_mode_changed_ = ui_->cbErase_when_mode_changed->isChecked();
  ...
  alert_USState_ = ui_->cbUSState->isChecked();
  alert_USStateOB_ = ui_->cbUSStateOB->isChecked();
...
//AE5TC - Handle ADIF log finished loading signal
void Configuration::impl::on_adif_log_loaded (int record_count, QString /*cty_version*/, QString const& /*error*/)
{
  adif_record_count_ = record_count;
  //AE5TC - Update button text with the count
  ui_->rescan_log_push_button->setText(QString{"Rescan ADIF Log (%1)"}.arg(adif_record_count_));
}
...
void Configuration::impl::on_callsign_states_download_button_clicked (bool /*clicked*/)
{
  ui_->callsign_states_download_button->setEnabled (false); // disable button until download is complete
  QDir dataPath {QStandardPaths::writableLocation (QStandardPaths::AppLocalDataLocation)};
  cty_download.configure(network_manager_,
//                         "https://timstools.com/ae5tc/callsign_lookup/states.php?format=tsv",
                         "http://risse.flammenkamp.de/hamlib/callsign_states.tsv",
                         dataPath.absoluteFilePath("callsign_states.tsv"),
                         "Downloading US Callsign States database");

  connect (&cty_download, &FileDownload::complete, this, &Configuration::impl::after_callsign_states_downloaded, Qt::UniqueConnection);
  connect (&cty_download, &FileDownload::error, this, &Configuration::impl::error_during_callsign_states_download, Qt::UniqueConnection);
  cty_download.start_download();
  ui_->callsign_states_label->setText("Downloading ...");
}

void Configuration::impl::error_during_callsign_states_download (QString const& reason)
{
  MessageBox::warning_message (this, tr ("Error Downloading Callsign States"), reason);
  ui_->callsign_states_download_button->setEnabled (true);
  read_callsign_states_info();
}

void Configuration::impl::after_callsign_states_downloaded()
{
    ui_->callsign_states_download_button->setEnabled(true);
    ui_->callsign_states_label->setText("Download complete");

    // Run TSV → binary conversion in a background thread
    QtConcurrent::run([=]() {

        USStates::ensureBinaryUpToDate();   // regenerate .bin if needed

        // Reload DB on UI thread once binary is ready
        QMetaObject::invokeMethod(this, [this]() {

            USStates::reloadCallsignDatabase();   // loads from binary
            USStates::preloadDatabaseAsync();     // optional warm-up

            read_callsign_states_info();          // update UI label
        });
    });
}

void Configuration::impl::read_callsign_states_info()
{
  QDir dataPath {QStandardPaths::writableLocation (QStandardPaths::AppLocalDataLocation)};
  QDir dataPath0 {QCoreApplication::applicationDirPath()};
  QString tsvPath = dataPath.absoluteFilePath("callsign_states.tsv");
  QFileInfo db {dataPath.absoluteFilePath("callsign_states.tsv")};
  QFileInfo db0 {dataPath0.absoluteFilePath("callsign_states.tsv")};
  if (db.exists()) {
    int size = db.size()/1024;
    QString lastMod = db.lastModified().toString("yyyy-MM-dd");
    ui_->callsign_states_label->setText(QString{"Callsign States DB: %1 KB (%2)"}.arg(size).arg(lastMod));
  } else if (db0.exists()) {
    int size = db0.size()/1024;
    QString lastMod = db0.lastModified().toString("yyyy-MM-dd");
    ui_->callsign_states_label->setText(QString{"Callsign States DB: %1 KB (%2)"}.arg(size).arg(lastMod));
  } else {
    ui_->callsign_states_label->setText("Callsign States DB: Not downloaded");
  }
}
...
void Configuration::impl::on_CALL3_download_button_clicked (bool /*clicked*/)
{
  ...
  cty_download.configure(network_manager_,
                         "https://risse.flammenkamp.de/hamlib/call3/CALL3.TXT",
...
void Configuration::impl::on_CALL3_EME_download_button_clicked (bool /*clicked*/)
{
  ...
  if (f.exists()) QFile::rename(dataPath.absolutePath() + "/" + "CALL3.TXT", dataPath.absolutePath() + "/" + "CALL3_backup.TXT");
  cty_download.configure(network_manager_,
                         "https://risse.flammenkamp.de/hamlib/call3/CALL3_EME.TXT",
                         dataPath.absoluteFilePath("CALL3.TXT"),
                         "Downloading latest CALL3.TXT file");
...
//                           "https://hamlib.sourceforge.net/snapshots-4.7/dll32/libhamlib-4.dll",  // new hamlib 4.7 download location
                           ("http://risse.flammenkamp.de/hamlib/dll32/" + hamlib_dll_basename + ".dll"),  // hamlib mirror DG2YCB
...
//                           "https://hamlib.sourceforge.net/snapshots-4.7/dll64/libhamlib-4.dll",  // new hamlib 4.7 download location
                           ("http://risse.flammenkamp.de/hamlib/dll64/" + hamlib_dll_basename + ".dll"),  // hamlib mirror DG2YCB
                           dataPath.absoluteFilePath(hamlib_dll_basename + "_new.dll"),
                           "Downloading latest Hamlib DLL file");
  }
...
void Configuration::impl::on_pbTestCloudlog_clicked ()
{
  ...
  cloudlog_.testApi(ui_->leCloudlogApiUrl->text(), ui_->leCloudlogApiKey->text(), ui_->sbCloudlogStationID->value());
}
