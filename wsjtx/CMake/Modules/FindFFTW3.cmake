...
# Threading library lookup is OPTIONAL — different toolchains package
# threading differently:
#   * MSYS2/mingw64: separate libfftw3f_threads.dll.a alongside libfftw3f
#     — must be linked explicitly because the Fortran `lib/downsam9.f90`,
#     `lib/filbig.f90` and friends call `fftwf_plan_with_nthreads`
#     unconditionally.
#   * JTSDK64: libfftw3f built with `--with-combined-threads` so threading
#     symbols are baked into the main fftw3f library — no separate
#     `_threads` library to find. Treating it as REQUIRED used to break
#     this toolchain (FindPackageHandleStandardArgs failed missing
#     FFTW3F_THREADS_LIBRARY).
# So: we attempt the find, append to FFTW3_LIBRARIES if found, and do
# NOT add to the required-args check list. Either layout works.
set (_thread_libs)
if (_use_threads)
...
# Search for all requested non-threading libraries (REQUIRED).
foreach (_lib ${_libraries})
...
# Search for threading libraries (OPTIONAL) — link them ahead of the
# main libs so the linker resolves fftwf_plan_with_nthreads from the
# threads library when it exists. If the threads library is missing
# (combined-threads build), fall through silently — the main library
# is expected to provide those symbols itself.
foreach (_lib ${_thread_libs})
  string (TOUPPER ${_lib} _LIB)
  find_library (${_LIB}_LIBRARY NAMES ${_lib} ${_lib}-3
    HINTS ${FFTW3_ROOT_DIR} PATH_SUFFIXES lib)
  mark_as_advanced (${_LIB}_LIBRARY)
